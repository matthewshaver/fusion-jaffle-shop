...
This is the foundation of a **cross-platform [dbt Mesh](https://docs.getdbt.com/docs/mesh/cross-platform-mesh)**: one project, one catalog, many engines.

---

## Prerequisites

### Accounts and access you'll need
...
2. **Bucket name**: pick a globally unique name. A good convention is `iceberg-<AWS_ACCOUNT_ID>-<REGION>`, e.g. `iceberg-859831564954-us-east-2`.
3. **Region**: choose `<REGION>` (e.g. `us-east-2`).
4. Leave the rest as defaults and click **Create bucket**.
5. (Optional but tidy) open the bucket and create a folder/prefix named `jaffle-iceberg/` — all Iceberg files will live under it.

### 2.2 Create the IAM permission policy

1. Go to **IAM → Policies → Create policy** and switch to the **JSON** tab.
2. Paste this, replacing the bucket name in both `Resource` blocks:
...
You can't fill in the real trust policy yet — it needs values Snowflake generates in Part 3. So create the role with a temporary trust, then fix it later.
...
Back in Snowsight (as `ACCOUNTADMIN`):
...
Now go back to **IAM → Roles → `snowflake-iceberg-role` → Trust relationships → Edit trust policy**, delete what's there, and paste this (with your two values):
...
> ```
>
> A JSON response containing `"access_token"` means you're good. If you see `Programmatic access token is invalid`, re-copy the token. If you see an error mentioning a network policy, see the FAQ.
...
> **Why `prod` and not `dev`?** This project's `stg_orders` model applies a `limit_in_dev` macro that filters orders to the last year **whenever the target is named `dev`**. The seed data is historical, so building on `dev` yields an **empty** `stg_orders` (and therefore empty `orders`). Building on any non-`dev` target skips that filter and gives you the full dataset. See the FAQ.
...
> **Why uppercase `alias`?** Snowflake stores unquoted identifiers in UPPERCASE, and Iceberg catalogs are case-sensitive. DuckDB quotes model names in lowercase, so without the uppercase alias it would look for `"stg_orders"` and miss `STG_ORDERS`. See the FAQ.
...
You should get a real row count (tens of thousands). You'll also see `ORDERS` listed as an **Iceberg table** in Snowsight under `DBT_ICEBERG.RAW` — written entirely by DuckDB.
...
## FAQ / Troubleshooting (everything we learned the hard way)

**Q: OAuth token exchange returns `invalid_scope` for every role, even `PUBLIC`.**
...
**Q: The exchange returns `Programmatic access token is invalid`.**
...
**Q: PAT creation or use fails complaining about a network policy.**
...
**Q: DuckDB gets `401 Unauthorized` from the catalog.**
...
**Q: DuckDB gets `404 Not Found` on `/v1/config`.**
...
**Q: The catalog returns the namespace but lists no tables, or DuckDB can't find a table it "should" see.**
...
**Q: `Catalog Error: Table with name "raw"."stg_orders" does not exist`.**
...
**Q: `SQL compilation error: Invalid time type scale ... TIMESTAMP_NTZ(9)`.**
Iceberg only supports microsecond timestamps. Cast to `timestamp_ntz(6)`.

**Q: `403 Forbidden` reading a `.metadata.json` from S3, with `region ''` / "No credentials provided".**
...
**Q: `403 Authorization failed` on `POST .../namespaces/RAW/tables` (create).**
...
**Q: `500 ... Object 'ORDERS' already exists as TABLE`.**
...
**Q: `unknown method: map has no method named warn_once` during grants.**
...
**Q: `Not implemented Error: Only ALTER TABLE is supported for Iceberg`.**
...
**Q: `Type with name number does not exist` (DuckDB suggests `numeric`).**
...
**Q: `orders` builds successfully but has 0 rows.**
...
**Q: `TRANSFORMER` can't run `count(*)` / "must specify a warehouse".**
...
**Q: How do I extend this beyond `orders`?**
